...
IMPORTANT: You need to finish the https://redhat-scholars.github.io/kube-native-java-apps/kube-native-java-apps/04-containers.html[Creating containers, window="_blank"] section before going into Kubernetes section.
...
After running this command at the `target/kubernetes` directory, Quarkus places `kubernetes.yaml` and `kubernetes.json` files to deploy the application.
One of the great things about the extension is the integration with container extension seen at https://redhat-scholars.github.io/kube-native-java-apps/kube-native-java-apps/04-containers.html[Creating containers, window="_blank"] section, so `image` tag is pointing out to the image created there.
